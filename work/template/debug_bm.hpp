...
#ifndef DEBUG_BM_PRINT_HPP
#define DEBUG_BM_PRINT_HPP

#ifdef LOCAL
#define debug_bm(...) debug_print::multi_print_bm(#__VA_ARGS__, __VA_ARGS__)
#else
#define debug_bm(...) ;
#endif
...
template <class Tp>
std::enable_if_t<!is_container_v<Tp>> out_bm(const Tp&);
void out_bm(const char&);
void out_bm(const char*);
void out_bm(const std::string_view&);

#if (defined _GLIBCXX_STRING) || (defined _LIBCPP_STRING)
void out_bm(const std::string&);
#endif

#ifdef __SIZEOF_INT128__
void out_bm(const __int128&);
void out_bm(const unsigned __int128&);
#endif

template <class Tp1, class Tp2>
void out_bm(const std::pair<Tp1, Tp2>&);

#if (defined _GLIBCXX_TUPLE) || (defined _LIBCPP_TUPLE)
template <class... Ts>
void out_bm(const std::tuple<Ts...>&);
#endif

#if (defined _GLIBCXX_STACK) || (defined _LIBCPP_STACK)
template <class... Ts>
void out_bm(std::stack<Ts...>);
#endif

#if (defined _GLIBCXX_QUEUE) || (defined _LIBCPP_QUEUE)
template <class... Ts>
void out_bm(std::queue<Ts...>);
template <class... Ts>
void out_bm(std::priority_queue<Ts...>);
#endif

template <class C>
std::enable_if_t<is_iteratable_container_v<C>> out_bm(const C&);

template <class Tp>
std::enable_if_t<!is_container_v<Tp>> out_bm(const Tp& arg) {
   std::cerr << arg;
}

void out_bm(const char& arg) {
   std::cerr << '\'' << arg << '\'';
}
...
void out_bm(const std::string_view& arg) {
   std::cerr << '\"' << arg << '\"';
}

#if (defined _GLIBCXX_STRING) || (defined _LIBCPP_STRING)
void out_bm(const std::string& arg) {
   std::cerr << '\"' << arg << '\"';
}
#endif

#ifdef __SIZEOF_INT128__
void out_bm(const __int128& arg) {
   int sign = (arg < 0) ? (-1) : 1;
   if (sign == -1)
      std::cerr << '-';
   __int128 base = sign;
...
void out_bm(const unsigned __int128& arg) {
...
template <class Tp1, class Tp2>
void out_bm(const std::pair<Tp1, Tp2>& arg) {
   std::cerr << '(';
   out_bm(arg.first);
   std::cerr << ", ";
   out_bm(arg.second);
...
void print_tuple(const T& arg, std::index_sequence<Is...>) {
   static_cast<void>(((std::cerr << (Is == 0 ? "" : ", "), out_bm(std::get<Is>(arg))), ...));
}

template <class... Ts>
void out_bm(const std::tuple<Ts...>& arg) {
   std::cerr << '(';
...
template <class... Ts>
void out_bm(std::priority_queue<Ts...> arg) {
   if (arg.empty()) {
...
std::enable_if_t<is_iteratable_container_v<Container>> out_bm(const Container& arg) {
...
template <class Tp, class... Ts>
void multi_print_bm(std::string_view names, const Tp& arg, const Ts&... args) {
   if constexpr (sizeof...(Ts) == 0) {
      ...
      multi_print_bm(names, args...);
   }
}
}  // namespace debug_print
#endif
#endif  // DEBUG_BM_PRINT_HPP
